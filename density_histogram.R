...
sizes <- temp_sizes %>%
  select("length", "y0", "y1", "y2") %>%
  gather(year_class, quantity, "y0", "y1", "y2") 
...
# plot 
p <- ggplot(temp_sizes_df, aes(x=temp_length, color=temp_class, fill=temp_class, position="dodge")) +  geom_histogram(aes(y=..density..), binwidth = 1.0, alpha=0.4, position="identity") + geom_density(alpha=0.2, bw = 0.75) + geom_vline(data = mu, aes(xintercept=grp_med, colour = temp_class), linetype = "dashed") + scale_color_manual(values=c("#a6611a","#dfc27d","#80cdc1"), guide = FALSE)  + scale_fill_manual(name = "Class",labels = c("Year0", "Year1", "Year2+"), values=c("#a6611a","#dfc27d","#80cdc1")) + theme(legend.position="top") + labs(title="Autumn 2020 Cockle Size Distribution", x="Size mm", y = "Frequency") + theme_classic()
